...
import { dayToursMeta, dayTours, getDayTour } from "@/lib/tours/day-tours";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const tour = getDayTour(slug);
  if (!tour) return { title: "Day Tour — GuimarasGo" };
  return { title: `${tour.title} — GuimarasGo`, description: tour.shortDescription };
}

export default async function DayTourDetailPage({ params }: PageProps) {
  const { slug } = await params;
  const tour = getDayTour(slug);
        ...
        <div className="mx-auto max-w-5xl px-4 pt-24 sm:px-6 sm:pt-28 lg:px-8">
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_20rem] lg:items-start">
            ...
            <ReservePanel
              ...
              reserveHref={`/book?category=${dayToursMeta.slug}&offering=${tour.slug}`}
            />
          </div>
        </div>
      </main>
